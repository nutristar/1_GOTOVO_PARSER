...
def rss_parser(xml: str, limit: Optional[int] = None, json=True) -> List[str]:
    ...
    if items:
        ...
        for i in items:
            items2 = {}
            try:                items2["Title" if json == False else "title"] = i.pop('title')
            except:                pass
            try:                items2["Author" if json == False else 'author'] = i['author']
            except:                pass
            try:                items2["Published" if json == False else 'pubDate'] = i['pubDate']
            except:                pass
            try:                items2["Link" if json == False else 'link'] = i['link']
            except:                pass
            try:                items2["Categories" if json == False else 'category'] = i['category']
            except:                pass
            try:                items2['description' if json == False else 'description'] = i['description']
            except:                pass
